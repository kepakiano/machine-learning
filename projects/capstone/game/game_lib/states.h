...
public:
  States(){}
  StatePtr getState(const std::list<CAsteroid> &asteroid_list,
  ...
  Environment loadStates(const int test_cases_id);
  void saveStates(const int test_cases_id);

  void printStatistics();


  std::unordered_map<StateHash, StatePtr> hashedStates() {return hashed_states_;}
...
private:
  std::unordered_map<StateHash, StatePtr> hashed_states_;
  ...
  void safeState(const StatePtr& state, const StateHash hash);
